...
CHOST=$(${SRC_DIR}/.build/${ctng_target_platform_short}/build/build-cc-gcc-final/gcc/xgcc -dumpmachine)
...
if [[ ${ctng_libc} == musl ]]; then
  ...
  make -l BUILD_CFLAGS="-O2 -g -I${SRC_DIR}/.build/${CHOST}/buildtools/include" \
          ...
          DESTDIR=${PREFIX}/${CHOST}/sysroot install
  popd
elif [[ ${ctng_libc} == newlib ]]; then
  pushd ${SRC_DIR}/.build/${CHOST}/build/build-libc/
  make -l BUILD_CFLAGS="-O2 -g -I${SRC_DIR}/.build/${CHOST}/buildtools/include" \
          BUILD_LDFLAGS="-L${SRC_DIR}/.build/${CHOST}/buildtools/lib"           \
          DESTDIR=${PREFIX}/${CHOST}/sysroot                                    \
          prefix="" exec_prefix="" tooldir="" build_tooldir="" install
  ls -la ${PREFIX}/${CHOST}/sysroot
  popd
fi
...
install -Dm644 $SRC_DIR/.build/src/gcc-*/COPYING.RUNTIME \
...
if [[ ${ctng_libc} == musl ]]; then
  if [[ ${ctng_cpu_arch} == riscv ]]; then
    ln -sf ../usr/lib/libc.so ${PREFIX}/${CHOST}/sysroot/lib/ld-musl-riscv32.so.1
  elif [[ ${ctng_cpu_arch} == openrisc ]]; then
    ln -sf ../usr/lib/libc.so ${PREFIX}/${CHOST}/sysroot/lib/ld-musl-or1k.so.1
  fi
fi
